          ...
          <div key={step.key} className="flex flex-1 items-start">
            <div className="flex flex-col items-center">
              ...
              <p className={`mt-2 text-center text-xs font-medium ${
                isBadOutcome ? 'text-red-500' : isCompleted ? 'text-gray-900' : isCurrent ? 'text-orange-600' : 'text-gray-400'
              }`}>
                {step.label}
              </p>
              {step.date && (
            ...
            {!isLast && (
              <div className={`mt-4 flex-1 border-t-2 ${
                idx < currentStepIndex ? 'border-green-400' : 'border-gray-200'
              }`} />
            )}
          </div>
        );
...
export default function ContractDetailPage() {
  ...
  return (
    <div className="mx-auto max-w-[1400px] p-6 lg:p-8">
      {/* Breadcrumb */}
      <Link href="/contracts" className="mb-6 flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 transition-colors w-fit">
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="mt-1 flex items-center gap-3 flex-wrap">
                ...
                <span className="text-sm text-gray-500">{contract.clientName}</span>
      ...
      <Card className="mb-6">
        <CardBody>
          <p className="mb-4 text-sm font-semibold text-gray-500">Contract Status</p>
          <StatusTimeline
              ...
              <div className="space-y-3">
                <div>
                  ...
                  {contract.clientContact && (
                    <p className="text-sm text-gray-500">{contract.clientContact}</p>
                  )}
                </div>
                {contract.clientEmail && (
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">Email</p>
                    <a href={`mailto:${contract.clientEmail}`} className="text-sm text-gray-700 hover:text-orange-600 transition-colors">
                      {contract.clientEmail}
                    </a>
                  </div>
                )}
            ...
            <CardBody>
              <div className="divide-y divide-gray-100">
                ...
                <div className="py-3 flex items-center justify-between">
                  ...
                  <span className="text-sm text-gray-700">{TEMPLATE_LABELS[contract.templateType]}</span>
                </div>
                {contract.terms?.amount && (
                  <div className="py-3 flex items-center justify-between">
                    <span className="text-xs font-semibold uppercase tracking-wide text-gray-400">Value</span>
                    <span className="font-bold text-gray-900">{formatCurrency(contract.terms.amount, contract.terms.currency)}</span>
                  </div>
                )}
                {contract.startDate && (
                  <div className="py-3 flex items-center justify-between">
                    <span className="text-xs font-semibold uppercase tracking-wide text-gray-400">Start Date</span>
                    <span className="text-sm text-gray-700">{formatDate(contract.startDate)}</span>
                  </div>
                )}
                {contract.endDate && (
                  <div className="py-3 flex items-center justify-between">
                    <span className="text-xs font-semibold uppercase tracking-wide text-gray-400">End Date</span>
                    <span className="text-sm text-gray-700">{formatDate(contract.endDate)}</span>
                  </div>
                )}
                <div className="py-3 flex items-center justify-between">
                  <span className="text-xs font-semibold uppercase tracking-wide text-gray-400">Created</span>
                  <span className="text-sm text-gray-700">{formatDate(contract.createdAt)}</span>
                </div>
                {contract.sentAt && (
                  <div className="py-3 flex items-center justify-between">
                    <span className="text-xs font-semibold uppercase tracking-wide text-gray-400">Sent</span>
                    <span className="text-sm text-gray-700">{formatDate(contract.sentAt)}</span>
                  </div>
                )}
                {contract.signedAt && (
                  <div className="py-3 flex items-center justify-between">
                    <span className="text-xs font-semibold uppercase tracking-wide text-gray-400">Signed</span>
              ...
              <div className="space-y-2">
                {contract.status === 'DRAFT' && (
                  <button
                    type="button"
                    onClick={handleSend}
                    className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors text-left"
                  >
