        ...
        <CardBody padding="lg">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              {client.contactName && (
                <p className="mt-1 text-sm text-gray-500">{client.contactName}</p>
              )}
            </div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
        <Card>
          <CardBody padding="lg">
            <div className="flex items-start justify-between">
              <p className="text-sm font-semibold uppercase tracking-wide text-gray-500">Total Invoices</p>
            ...
            <p className="mt-3 text-3xl font-bold text-gray-900">{client.totalInvoices}</p>
            <p className="mt-1 text-sm text-gray-500">invoices sent</p>
          </CardBody>
        </Card>
        ...
        <Card>
          <CardBody padding="lg">
            ...
            <p className="mt-3 text-3xl font-bold text-gray-900">{formatCurrency(client.totalRevenue, client.currency)}</p>
        ...
        <Card>
          <CardBody padding="lg">
            <div className="flex items-start justify-between">
              ...
              <div className={`flex h-9 w-9 items-center justify-center rounded-lg ${client.outstandingBalance > 0 ? 'bg-orange-50' : 'bg-gray-50'}`}>
            ...
            <p className={`mt-3 text-3xl font-bold ${client.outstandingBalance > 0 ? 'text-orange-600' : 'text-gray-900'}`}>
              {formatCurrency(client.outstandingBalance, client.currency)}
            </p>
            <p className="mt-1 text-sm text-gray-500">{client.outstandingBalance > 0 ? 'awaiting payment' : 'fully paid up'}</p>
          </CardBody>
        </Card>
        ...
        <Card>
          <CardBody padding="lg">
            <div className="flex items-start justify-between">
              <p className="text-sm font-semibold uppercase tracking-wide text-gray-500">Overdue</p>
              <div className={`flex h-9 w-9 items-center justify-center rounded-lg ${client.overdueCount > 0 ? 'bg-red-50' : 'bg-gray-50'}`}>
            ...
            <p className={`mt-3 text-3xl font-bold ${client.overdueCount > 0 ? 'text-red-600' : 'text-gray-900'}`}>
              {client.overdueCount}
            </p>
            <p className={`mt-1 text-sm font-medium ${client.overdueCount > 0 ? 'text-red-500' : 'text-gray-400'}`}>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200">
                      <th className="pb-3 text-left font-semibold text-gray-700">Invoice #</th>
                      <th className="pb-3 text-left font-semibold text-gray-700">Issue Date</th>
                      <th className="pb-3 text-left font-semibold text-gray-700">Due Date</th>
                      <th className="pb-3 text-right font-semibold text-gray-700">Amount</th>
                      <th className="pl-4 pb-3 text-left font-semibold text-gray-700">Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    {invoices.map((inv) => (
                      <tr key={inv.id} className="border-b border-gray-100 last:border-0">
                        ...
                        <td className="py-3 text-gray-700">{formatDate(inv.issueDate)}</td>
                        <td className="py-3 text-gray-700">{formatDate(inv.dueDate)}</td>
          ...
          <Card>
            ...
            <CardBody>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Health Score</p>
                  <div className="mt-2">
              ...
              <div className="mt-4 space-y-2 text-sm">
                <div className="flex justify-between text-gray-600">
                  ...
                  <span className={client.overdueCount > 0 ? 'font-semibold text-red-600' : 'text-gray-900'}>
                    {client.overdueCount}
                  </span>
                </div>
                <div className="flex justify-between text-gray-600">
              ...
              <div className="divide-y divide-gray-100">
                {client.email && (
                  <div className="py-3 first:pt-0 last:pb-0">
                    ...
                    <a href={`mailto:${client.email}`} className="mt-1 block text-sm text-gray-700 hover:text-orange-600">
                      {client.email}
                    </a>
                  </div>
                )}
                {client.phone && (
                  <div className="py-3 first:pt-0 last:pb-0">
                    ...
                    <p className="mt-1 text-sm text-gray-700">{client.phone}</p>
                  </div>
                )}
                {formatAddress() && (
                  <div className="py-3 first:pt-0 last:pb-0">
                    <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">Billing Address</p>
                    <p className="mt-1 whitespace-pre-line text-sm text-gray-700">{formatAddress()}</p>
                  </div>
                )}
                <div className="py-3 first:pt-0 last:pb-0">
                  <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">Payment Terms</p>
                  <p className="mt-1 text-sm text-gray-700">{formatPaymentTerms(client.paymentTerms)}</p>
                </div>
                <div className="py-3 first:pt-0 last:pb-0">
                  <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">Currency</p>
                  <p className="mt-1 text-sm text-gray-700">{client.currency}</p>
                </div>
                {client.notes && (
                  <div className="py-3 first:pt-0 last:pb-0">
